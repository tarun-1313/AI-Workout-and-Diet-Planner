...
2.  Add your API key to your environment variables or directly into `config.yaml` (though environment variables are recommended for security).

    Example `config.yaml` entry (if not using environment variables):
    ```yaml
    gemini:
